...
static int crypto_aes_ofb_op(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	...
	do {
		/* write plaintext to AESAXDIN; hardware XORs with keystream and triggers cipher */
		ret = aes_load_xor_data_in(config->regs, &pkt->in_buf[bytes_processed]);
		if (ret != 0) {
			break;
		}
...
static int crypto_aes_ctr_op(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	const struct device *dev = ctx->device;
	const struct crypto_mspm0_aes_config *config = dev->config;
	struct mspm0_aes_session *session = ctx->drv_sessn_state;
	struct crypto_mspm0_aes_data *data = dev->data;
	uint8_t nonce_counter[AES_BLOCK_SIZE] __aligned(4);
	uint8_t nonce_len;
	uint32_t counter = 0;
	int bytes_processed = 0;
	int ret;
	...
	if (iv == NULL) {
		LOG_ERR("Counter input is invalid");
		return -EINVAL;
	}
	...
	/* ivlen(nonce) + ctr_len = keylen */
	nonce_len = AES_BLOCK_SIZE - ctx->mode_params.ctr_info.ctr_len / 8;
	memcpy(nonce_counter, iv, nonce_len);

	do {
		/* nonce || counter */
		sys_put_be32(counter, &nonce_counter[nonce_len]);

		/* encrypt */
		ret = aes_load_data_in(config->regs, nonce_counter);
		if (ret != 0) {
			break;
		}
		...
		/* XOR encrypted counter with plaintext via AESAXIN (no retrigger) */
		ret = aes_load_xor_data_in_without_trigger(config->regs,
			     ...
			     enum cipher_algo algo, enum cipher_mode mode, enum cipher_op op)
{
	...
	case CRYPTO_CIPHER_MODE_OFB:
		aesconfig = AES_MSPM0_AESACTL0_CMX_OFB |
			    ((op == CRYPTO_CIPHER_OP_ENCRYPT)
			    ? AES_MSPM0_AESACTL0_OPX_ENCRYPT
			    : AES_MSPM0_AESACTL0_OPX_DECRYPT);
	...
	case CRYPTO_CIPHER_MODE_CTR:
		aesconfig = AES_MSPM0_AESACTL0_CMX_ECB | AES_MSPM0_AESACTL0_OPX_ENCRYPT;
