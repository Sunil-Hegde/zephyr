...
typedef struct {
	...
	volatile uint32_t iset;		/* Interrupt Set			*/
	volatile uint32_t iclr;		/* Interrupt Clear			*/
} rtc_ti_mspm0_gen_event_int_reg_t;

typedef struct {
	uint32_t reserved6[0x111];
	volatile uint32_t fpub_0;	/* Publisher Port 0			@0x444h */
	uint32_t reserved7[0xEE];
	...
	uint32_t reserved9[0x1FB];
	volatile uint32_t clksel;	/* Clock Select				@0x1004h */
	uint32_t reserved10[6];
	rtc_ti_mspm0_cpu_int_reg_t cpu_int;		/* @0x1020h */
	rtc_ti_mspm0_gen_event_int_reg_t gen_event;	/* @0x1050h */
	uint32_t reserved11[0x1E];
					   ...
					   const struct rtc_time *timeptr)
{
	...
	cfg->regs->cpu_int.imask &= ~(RTC_MSPM0_IMASK_ALARM1);
					   ...
					   const struct rtc_time *timeptr)
{
	...
	cfg->regs->cpu_int.imask &= ~(RTC_MSPM0_IMASK_ALARM2);
